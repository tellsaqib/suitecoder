    ...
    CodeMirror.simpleHint = function(editor, getHints, givenOptions) {
        ...
        function collectHints(previousToken) {
            ...
            function insert(str) {
                ...
                var stringToReplace = str;
                //By Saqib
                //making sure that we are not removing intial quote
                var plusOnePosition = {
                    ch: (result.from.ch + 1),
                    line: result.from.line
                };

                var firstChar = editor.getRange(result.from,plusOnePosition);
                if(firstChar == '"')
                ...
                editor.replaceRange(stringToReplace, result.from, result.to);
            }
